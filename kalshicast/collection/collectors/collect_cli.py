    ...
    *,
    ...
    for loc in loc_ids:
        try:
            ...
            if not items:
                continue # No products here, check next loc_id
                ...
                if had_letter:
                    ...
                    if fb:
                        ...
                        if (abs(high - fb_high) > 20.0) or (abs(low - fb_low) > 20.0):
                            # Guardrail tripped: reject this CLI parsing and keep looking
                            continue

                return high, low, text, issued_at

        except requests.RequestException as e:
            # Let network timeouts bubble up so the outer loop actually retries the HTTP request
            raise e
        except Exception:
            # Parsing error, just try the next loc_id
            continue

    # If we made it through all loc_ids and found nothing, return None to trigger fallback.
    return None
...
def fetch_observations_for_station(*, conn, run_id, station: dict, target_date: str) -> bool:
    ...
    loc_ids: List[str] = []
    if cli_site: loc_ids.append(cli_site)
    if station.get("cli_location_id"): loc_ids.append(str(station["cli_location_id"]).upper().strip())
    if station_id.startswith("K") and len(station_id) == 4: loc_ids.append(station_id[1:])
    ...
    for attempt in range(1, OBS_MAX_ATTEMPTS + 1):
        try:
            # Will return None if no data exists, will RAISE if there's a 503/Timeout
            ...
            # If hi_lo IS None, the data isn't there. Fallback immediately!
            fb = _fallback_station_obs(station, target_date)
            if fb:
                high, low = fb
                upsert_observation(
            ...
            # If the fallback is ALSO empty, trigger the retry loop
            raise ValueError("CLI missing and station hourly observations are empty")

        except Exception as e:
            last_err = e
            retryable = _is_retryable_http(e) or any(
                s in str(e).lower() for s in ("timed out", "timeout", "temporarily", "try again", "connection reset", "service unavailable")
            )
...
def _fetch_worker(run_id: str, station: dict, target_date: str) -> bool:
    """Thread worker that handles its own database connection safely.

    NOTE: oracledb's context manager commits/rolls back on exit but does NOT
    close the connection. We use an explicit try/finally to ensure the
    connection is always closed, avoiding connection leaks across threads.
    """
    conn = get_conn()
    try:
        ok = fetch_observations_for_station(conn=conn, run_id=run_id, station=station, target_date=target_date)
        conn.commit()
        return ok
    except Exception as e:
        try:
            ...
            pass
        print(f"[obs] FAIL {station.get('station_id')} {target_date}: {e}", flush=True)
        return False
    finally:
        conn.close()
...
def fetch_observations(stations: list, target_date: str) -> bool:
    ...
    # 2. Launch 5 stations at the same time
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as ex:
        futures = [ex.submit(_fetch_worker, run_id, st, target_date) for st in stations]

        # 3. As each station finishes, check if it succeeded (any_ok tracking)
        for fut in concurrent.futures.as_completed(futures):
            if fut.result(): # If the worker returned True
                any_ok = True

    # 4. Return exactly like your old code
    return any_ok
